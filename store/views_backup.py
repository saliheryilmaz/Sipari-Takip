...
def dashboard(request):
    ...
    tire_sales_data = LastikEnvanteri.objects.filter(
        durum='TESLIM_EDILDI'  # Teslim edilmiş lastikler = satış
    )
...
class LastikEnvanteriListView(LoginRequiredMixin, ExportMixin, SingleTableView):
    ...
    def get_queryset(self):
        ...
        if cari:
            queryset = queryset.filter(cari__icontains=cari)
...
class LastikEnvanteriUpdateView(LoginRequiredMixin, UpdateView):
    ...
    template_name = 'store/lastik_envanteri_form.html'
    fields = ['cari', 'urun', 'marka', 'grup', 'mevsim', 'adet', 'birim_fiyat', 'toplam_fiyat',
              'ambar', 'aciklama1', 'odeme', 'durum', 'sms_gonderildi']
    success_url = '/lastik-envanteri'
    login_url = 'login'
...
def lastik_dashboard(request):
    ...
    # Önce tüm markaları büyük harfe çevirip grupla
    marka_dagilimi_raw = LastikEnvanteri.objects.annotate(
        marka_upper=Upper('marka')
    ).values('marka_upper').annotate(count=Count('id')).order_by('-count')[:10]
    ...
    # Chart verileri için
    # Lastik satış analizi - mevsim ve araç tipine göre
    tire_sales_data = LastikEnvanteri.objects.filter(
        durum='TESLIM_EDILDI'  # Teslim edilmiş lastikler = satış
    )
    tire_sales_by_season_group = {}
    ...
    # Marka dağılımı için chart verileri
    lastik_marka_labels = [item['marka'] for item in marka_dagilimi]
    lastik_marka_values = [item['count'] for item in marka_dagilimi]
    
    # Ödeme durum dağılımı
    lastik_odeme_counts = LastikEnvanteri.objects.values('odeme').annotate(count=Count('id'))
    lastik_odeme_labels = [item['odeme'] or "Belirtilmemiş" for item in lastik_odeme_counts]
    lastik_odeme_values = [item['count'] for item in lastik_odeme_counts]
    
    # Sales over time (basit veri)
    sale_dates_labels = ['2024-01', '2024-02', '2024-03', '2024-04', '2024-05', '2024-06']
    sale_dates_values = [10, 15, 8, 12, 20, 18]
...
def whatsapp_gonder(request, lastik_id):
    ...
    try:
        ...
        mesaj = f"""
*Lastik Envanteri - {lastik.cari}*

*Ürün:* {lastik.urun}
*Marka:* {lastik.marka}
*Adet:* {lastik.adet}
*Durum:* {lastik.get_durum_display()}
*Güncellenen Son Tarih:* {lastik.guncelleme_tarihi.strftime('%d.%m.%Y %H:%M')}
        """
